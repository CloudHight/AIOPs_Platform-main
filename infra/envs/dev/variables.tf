...
variable "environment" {
  ...
  validation {
    ...
    error_message = "This root module is for the dev environment only."
  }
}
...
variable "enable_aiops_control_plane" {
  description = "Whether to deploy the AIOps control plane in dev."
  type        = bool
  default     = false
}
...
variable "jira_secret_name" {
  description = "Optional explicit Jira credentials secret name. Defaults to an account-scoped dev name to avoid scheduled-deletion conflicts."
  type        = string
  default     = null
}

variable "jira_secret_recovery_window_in_days" {
  description = "Recovery window for the Jira credentials secret. Dev defaults to immediate deletion for repeatable ephemeral teardown."
  type        = number
  default     = 0
